...
19) 한 웨이브에 설정된 모든 적을 처치하면 다음 웨이브로 넘어간다.
20) 웨이브가 지날 수록 적군의 체력과 공격력이 강해지며 처음에는 나오지 않던 강한 종류의 적군이 등장한다.
21) 주어진 모든 웨이브를 버텨내면 승리하며 플레이어의 체력이 모두 소모되면 패배한다.
22) 승리 또는 패배 시 다시 스테이지 화면으로 돌아간다.
<br>
