...
3) 오브젝트 이름 : 게임 시스템 관리 오브젝트

|현상태|전이상태|전이조건|  
|:---|:---|:---|  
|휴식|전투|정해진 휴식 시간이 지났을 때 전투 돌입|
|전투|휴식|정해진 전투 시간이 지났을 때 휴식 상태로 전환|
...
3.전투 시간이 끝나면 약간의 휴식 시간이 제공된다. 이 휴식시간동안 교단을 통해 플레이어를 강화시킬수 있다.  
4.주어진 웨이브를 모두 통과해 생존하면 승리  

2) 보조 규칙  
전투 중과 휴식중 사용할 수 있는 아이템이 나눠져 있다.  
전투중 사용할 수 있는 아이템은 수집 즉시 효과가 나타나며 생존에 도움을 준다.  
휴식중 사용할 수 있는 아이템은 장기적으로 플레이어를 큰폭으로 성장 시켜준다.  
...
- 시작화면에는 게임 시작과 도전 과제, 게임 종료라는 버튼 3개가 있다.  
- 게임 시작 클릭 시 게임화면으로 이동한다.  
- 게임 종료 클릭 시 게임을 종료한다.  
- 도전 과제 클릭 시 게임을 플레이하면서 해결해야하는 과제 목록을 보여준다.  
- 도전 과제는 게임 플레이 자체에는 영향을 주지않으며 도전 과제 해결시 숨겨진 스토리를 보여준다.  
- 점수 화면에서는 기존에 기록한 하이스코어 5개를 보여준다.  
- 점수보기 화면에는 돌아가기 버튼이 있고 클릭 시 시작화면으로 이동한다.  
- 시작화면에서 게임 시작을 누르면 교단 선택 화면으로 변경된다.  
- 교단 선택화면에서는 6개의 교단과 각 교단의 심볼이 표시된다.
- 심볼을 누르면 교단장의 그림과 함께 플레이어에게 주어지는 스킬과 설명, 교단의 특색이 표시된다.  
- 교단을 모두 선택하면 게임화면으로 이동한다.  
- 게임 화면의 가운에 상단에는 전투시간 또는 휴식시간이 표시된다.  
- 게임 화면의 우측 상단에는 현재 웨이브 수가 표시된다.  
- 웨이브 수 밑에는 전투 중 획득한 아이템이 표시된다.  
- 게임시작시 화면 가운데에 Wave Start!라는 문구가 나오며 전투 기간으로 돌입한다.  
- 마우스의 방향에 따라 플레이어가 이동하며 카메라는 플레이어를 따라다닌다.  
- 카메라에서 화면의 폭, 높이를 구해 화면 바깥에서 적군이 생성되게 한다.  
...
- 전투 중에는 화면 바깥에서 아이템도 생성되며 전투 기간 전용 아이템과 휴식 기간 전용 아이템 2종류가 있다.  
- 전투 아이템은 습득 즉시 효과가 발동하며 휴식 아이템은 휴식 기간에만 사용할 수 있다.  
- 한 웨이브에 설정된 모든 적을 처치하면 휴식 기간으로 넘어간다.  
- 휴식 기간에는 교단에게 전투 중 얻은 아이템을 사용하여 플레이어를 강화시킬 수 있다.  
- 휴식 기간이 종료되면 다시 전투 기간으로 돌입하고 웨이브가 시작된다.
- 웨이브가 지날 수록 적군의 체력과 공격력이 강해지며 처음에는 나오지않던 강한 종류의 적군이 등장한다.
