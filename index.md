...
[도전 과제]
1) 몰려 오는 적들을 공격과 스킬을 사용해 처치
2) 스테이지내에 모든 웨이브의 적들을 모두 처치 할 때 까지 스테이지를 진행
3) 시간이 지날수록 강한 적이 생성
4) 강한 적을 처치할 수록 더 많은 자원을 얻음
5) 자원을 통해 플레이어가 성장
...
|속성|속성값|설명|비고|  
|:---|:---|:---|:---|  
|공격력|10f|공격이 입히는 피해량|수치가 높아질 수 있음|  
...
|재장전 시간|2f~5f|스킬을 다시 사용하기까지 걸리는 시간|스킬마다 다른 수치를 가짐|  
...
핵심 규칙  
1) 게임 시작전 주교단과 부교단을 선택한다.
2) 주교단은 무기를, 부교단은 기술을 제공한다.
3) 무기는 공격(좌클릭)을 변화시키며 기술은 스킬(휠클릭)을 변화시킨다.
4) 게임 좌측 상단에 웨이브수가 표시되고 플레이어는 공격과 스킬을 사용해 몰려오는 적들을 처치한다.   
5) 주어진 웨이브를 모두 통과해 생존하면 승리  
...
### 화면
01) 시작 화면, 스테이지 화면, 준비 화면, 게임화면, 도전과제 화면 총 5개의 화면이 있다.
2) 시작 화면에는 게임시작, 도전과제, 게임종료 총 버튼 3개가 있다.
...
17) 마우스의 방향에 따라 플레이어가 이동하며 카메라는 플레이어를 따라다닌다.
...
21) 적군을 처치하면 경험치가 오르며 경험치가 모두 차면 레벨 업이 된다.
...
25) 한 웨이브에 설정된 모든 적을 처치하면 다음 웨이브로 넘어간다.
...
## 요구사항(6주)
### 화면
1) 시작 화면, 준비 화면, 게임화면 총 3개의 화면이 있다.
2) 시작 화면에는 게임시작, 게임종료 총 버튼 2개가 있다.
3) 게임 종료 클릭 시 게임을 종료한다.
4) 시작화면에서 게임시작 클릭 시 준비 화면으로 이동한다.
5) 준비 화면에서는 2개 교단의 심볼, 플레이 버튼이 표시된다.
6) 심볼을 누르면 교단장의 그림과 함께 플레이어에게 주어지는 스킬과 설명, 교단의 특색이 표시된다.
7) 플레이 버튼 클릭 시 게임 화면으로 이동한다.
<br>

### UI
8) 게임 화면의 우측 상단에는 현재 웨이브 수가 표시된다.
9) 화면의 가운데 하단에는 플레이어의 경험치 체력 스킬 아이콘과 재사용 대기시간이 표시된다.
10) 스킬 아이콘은 교단의 심볼과 동일하다.
11) 레벨 업 시 2개의 교단중 하나가 랜덤으로 선택되어 해당 교단의 3개의 선택지가 생성된다.
<br>

### 게임 플레이 
12) 게임 시작 시 화면 가운데에 Wave Start!라는 문구가 나오며 전투에 돌입한다.
13) 마우스의 방향에 따라 플레이어가 이동하며 카메라는 플레이어를 따라다닌다.
14) 카메라에서 화면의 폭, 높이를 구해 화면 바깥의 랜덤한 위치에서 적군이 생성되게 한다.
15) 적군은 플레이어를 자동으로 추적하며 공격을 통해 플레이어에게 가지고있는 공격력만큼 피해를 준다.
16) 플레이어는 마우스 좌,휠 클릭을 사용해 처음에 선택했던 교단의 무기와 스킬을 사용할 수 있다.
17) 적군을 처치하면 경험치가 오르며 경험치가 모두 차면 레벨 업이 된다.
18) 선택지를 통해서 교단의 스킬이나 플레이어의 기본 능력치를 강화 시킬 수 있다.
19) 교단에 따라 강화 시킬 수 있는 플레이어의 기본 능력치가 각각 다르다.
20) 한 웨이브에 설정된 모든 적을 처치하면 다음 웨이브로 넘어간다.
21) 웨이브가 지날 수록 적군의 체력과 공격력이 강해지며 처음에는 나오지 않던 강한 종류의 적군이 등장한다.
22) 주어진 모든 웨이브를 버텨내면 승리하며 플레이어의 체력이 모두 소모되면 패배한다.
23) 승리 또는 패배 시 다시 스테이지 화면으로 돌아간다.
<br>
